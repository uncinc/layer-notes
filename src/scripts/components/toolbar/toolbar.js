      ...
      buttons: [
        {
          ...
          text: 'New note', // the button title
          title: 'Create a new note', // the title hover. So the user will get a hint
    ...
    return (
      <Anime delay={(e, i) => i * 10} duration={500} translateX={this.state.translateX} translateY={['-50%', '-50%']} easing={'easeOutBack'}>
        <section className={'ln-toolbar elasticity-100'}>
          {this._renderButtons()}
          <span className="ln-toolbar-amouttickets" title="Amount tickets on this page, and the total tickets on this site.">{this.props.filtertTotal}/{this.props.total}</span>
        </section>
      </Anime>
    );
  }
